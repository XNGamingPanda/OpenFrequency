    ...
    function enqueueAudioPlayback(audioUrl, volume = 1.0) {
        playbackQueue.push({ audioUrl, volume });
        if (!isPlaybackActive) {
            playNextQueuedAudio();
        }
    }
    ...
    function playNextQueuedAudio() {
        ...
        const { audioUrl, volume } = playbackQueue.shift();
    ...
    socket.on('chatter_audio', (data) => {
        try {
            ...
            const audioBlob = new Blob([audioBytes], { type: 'audio/mpeg' });
            const audioUrl = URL.createObjectURL(audioBlob);
            enqueueAudioPlayback(audioUrl, chatterVolume);
        } catch (e) { console.error(e); }
    });
