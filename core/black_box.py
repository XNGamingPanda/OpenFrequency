...
class BlackBox:
    ...
    def _end_flight(self, final_record):
        ...
        if REPORTING_AVAILABLE:
            threading.Thread(target=self._generate_report_thread, args=(flight_duration, final_record), daemon=True).start()
        else:
            # Always generate a basic JSON report even without pandas/matplotlib
            threading.Thread(target=self._generate_basic_report, args=(flight_duration, final_record), daemon=True).start()

    def _generate_basic_report(self, duration, final_record):
        """Generate a basic JSON flight report without pandas/matplotlib."""
        try:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            report_id = f"flight_{timestamp}"
            data = list(self.flight_data)
            if self.flight_start_time:
                data = [r for r in data if r.get('timestamp', 0) >= self.flight_start_time]

            max_alt = max((r.get('altitude', 0) for r in data), default=0)
            max_spd = max((r.get('airspeed', 0) for r in data), default=0)
            max_vs  = max((abs(r.get('vs', 0)) for r in data), default=0)
            ...
            report = {
                ...
                'max_altitude_ft': round(max_alt),
                'max_airspeed_kt': round(max_spd),
                'max_vs_fpm':      round(max_vs),
                'landing_g':       landing.get('g_force'),
                'landing_vs':      landing.get('vs'),
                'landing_speed':   landing.get('touchdown_speed'),
                'bounces':         landing.get('bounces', 0),
                'data_points':     len(data),
            }

            os.makedirs(self.data_dir, exist_ok=True)
            json_path = os.path.join(self.data_dir, f"{report_id}.json")
            with open(json_path, 'w', encoding='utf-8') as f:
                json.dump(report, f, ensure_ascii=False, indent=2)

            print(f"BlackBox: Basic report saved 鈫?{json_path}")
            event_bus.emit('flight_report_ready', {'report_id': report_id, 'report': report, 'type': 'basic'})
        except Exception as e:
            print(f"BlackBox: Basic report generation failed: {e}")
    ...
    def _generate_report_thread(self, duration, final_record):
        ...
        try:
            ...
            if g_force < 1.2:
                ...
                pax_comment = "Total Butter! Did we even touch the ground? 馃憦"
                score = 100
            elif g_force < 1.5:
                pax_mood = "Happy"
                ...
                pax_mood = "Terminated"
                pax_comment = "My coffee is on the ceiling! 馃槺"
                score = 40
            else:
                pax_mood = "Traumatized"
                pax_comment = "Grandma's dentures flew into the cockpit. I'm suing! 馃殤"
                score = 0
            ...
            <body>
                <div class="container">
                    <h1 class="mb-4">鉁堬笍 Flight Report <small class="text-muted">{timestamp}</small></h1>
